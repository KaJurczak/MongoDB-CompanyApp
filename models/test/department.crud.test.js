...
describe('Department', () => {
  ...
  describe('Creating data', () => {
    it('should insert new document with "insertOne" method', async () => {
      ...
      expect(department.isNew).to.be.false; // instead of this:
  ...
  describe('Removing data', () => {
    ...
    it('should properly remove multiple documents with "deleteMany" method', async () => {
      await Department.deleteMany();
      const removedDepartment = await Department.findOne({ name: 'Department #1' });
      const removedDepartmentTwo = await Department.findOne({ name: 'Department #2' });
      expect(removedDepartment).to.be.null;
      expect(removedDepartmentTwo).to.be.null;
    });
