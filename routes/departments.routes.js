...
router.get('/departments/:id', (req, res) => {
  req.db.collection('departments').findOne({ _id: ObjectId(req.params.id) }, (err, data) => { //ObjectId convert req.params.id from type string to objectId (same as _id) 
    if(err) res.status(500).json({ message: err });
...
router.put('/departments/:id', (req, res) => {
  const { name } = req.body;
  req.db.collection('departments').updatetOne({ _id: ObjectId(req.params.id) }, { $set: { name: name }}, err => {
    if(err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
